...
// Custom Feature: Onboarding compliance auditing
// Onboarding compliance auditing: Background process checks onboarding completion rates by department; flags compliance gaps (e.g., missing security training)
import React, { useState } from 'react';

export default function CFOnboardingComplianceAuditingPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-onboarding-compliance-auditing/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Onboarding compliance auditing</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Onboarding compliance auditing: Background process checks onboarding completion rates by department; flags compliance gaps (e.g., missing security training)</p>
