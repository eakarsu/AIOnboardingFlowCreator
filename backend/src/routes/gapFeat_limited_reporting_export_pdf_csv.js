...
// Gap Feature: Limited reporting export (PDF/CSV)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIOnboardingFlowCreator / gap-limited-reporting-export-pdf-csv',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Limited reporting export (PDF/CSV)'.\nProject: AIOnboardingFlowCreator.\nContext: Limited reporting export (PDF/CSV)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['limited-reporting-export-pdf-csv', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'limited-reporting-export-pdf-csv', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
