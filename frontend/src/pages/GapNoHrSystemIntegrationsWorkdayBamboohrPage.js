...
// Gap (Non-AI): No HR system integrations (Workday, BambooHR)
// No HR system integrations (Workday, BambooHR)
import React, { useState } from 'react';

export default function GapNoHrSystemIntegrationsWorkdayBamboohrPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-hr-system-integrations-workday-bamboohr/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No HR system integrations (Workday, BambooHR)</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No HR system integrations (Workday, BambooHR)</p>
