...
// Custom Feature: Real-time feedback loop
// Real-time feedback loop: Video/audio captures during onboarding sessions; AI transcribes and surfaces common questions (e.g., \"Where's the bathroom?\" → add to FAQ) and sentiment (frustration indicators)
import React, { useState } from 'react';

export default function CFRealTimeFeedbackLoopPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/cf-real-time-feedback-loop/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Real-time feedback loop</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Real-time feedback loop: Video/audio captures during onboarding sessions; AI transcribes and surfaces common questions (e.g., \"Where's the bathroom?\" → add to FAQ) and sentiment (frustration indicators)</p>
