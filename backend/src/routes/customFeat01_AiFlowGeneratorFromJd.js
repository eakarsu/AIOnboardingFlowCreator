...
// Upload job description; AI generates custom onboarding flow with tailored checklists, learning resources, and mentor pairings
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIOnboardingFlowCreator / cf-ai-flow-generator-from-jd',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_ai_flow_generator_from_jd`;
(async () => {
  try {
    await pool.query(`
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'AI flow generator from JD'.
Context: Upload job description; AI generates custom onboarding flow with tailored checklists, learning resources, and mentor pairings
...
If input is insufficient, populate sensible defaults and note assumptions in summary.`;
    ...
    console.error('cf-ai-flow-generator-from-jd/run error:', err);
