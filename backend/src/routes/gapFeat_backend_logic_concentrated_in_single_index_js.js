...
// Gap Feature: Backend logic concentrated in single index.js
// Backend logic concentrated in single index.js — no modular route files
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIOnboardingFlowCreator / gap-backend-logic-concentrated-in-single-index-js',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Backend logic concentrated in single index.js'.\nProject: AIOnboardingFlowCreator.\nContext: Backend logic concentrated in single index.js — no modular route files\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['backend-logic-concentrated-in-single-index-js', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'backend-logic-concentrated-in-single-index-js', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['backend-logic-concentrated-in-single-index-js']
    );
