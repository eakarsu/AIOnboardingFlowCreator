...
// Gap Feature: No HR system integrations (Workday, BambooHR)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIOnboardingFlowCreator / gap-no-hr-system-integrations-workday-bamboohr',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No HR system integrations (Workday, BambooHR)'.\nProject: AIOnboardingFlowCreator.\nContext: No HR system integrations (Workday, BambooHR)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-hr-system-integrations-workday-bamboohr', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-hr-system-integrations-workday-bamboohr', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
