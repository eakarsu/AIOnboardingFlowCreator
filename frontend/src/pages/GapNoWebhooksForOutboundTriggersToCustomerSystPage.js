...
// Gap (Non-AI): No webhooks for outbound triggers to customer systems
// No webhooks for outbound triggers to customer systems
import React, { useState } from 'react';

export default function GapNoWebhooksForOutboundTriggersToCustomerSystPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-webhooks-for-outbound-triggers-to-customer-syst/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No webhooks for outbound triggers to customer systems</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No webhooks for outbound triggers to customer systems</p>
