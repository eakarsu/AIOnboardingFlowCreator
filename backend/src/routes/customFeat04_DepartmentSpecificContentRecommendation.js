...
// AI learns which content (training videos, docs, peer mentors) best aids specific role types; recommends resources proactively
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIOnboardingFlowCreator / cf-department-specific-content-recommendation',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_department_specific_content_recommendation`;
(async () => {
  try {
    await pool.query(`
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Department-specific content recommendation'.
Context: AI learns which content (training videos, docs, peer mentors) best aids specific role types; recommends resources proactively
...
If input is insufficient, populate sensible defaults and note assumptions in summary.`;
    ...
    console.error('cf-department-specific-content-recommendation/run error:', err);
