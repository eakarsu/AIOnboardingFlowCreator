...
// AI tracks learner engagement (quiz scores, time-on-page); auto-adjusts flow (skip easy sections, slow down hard ones)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIOnboardingFlowCreator / cf-adaptive-pacing',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_adaptive_pacing`;
(async () => {
  try {
    await pool.query(`
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Adaptive pacing'.
Context: AI tracks learner engagement (quiz scores, time-on-page); auto-adjusts flow (skip easy sections, slow down hard ones)
...
If input is insufficient, populate sensible defaults and note assumptions in summary.`;
    ...
    console.error('cf-adaptive-pacing/run error:', err);
