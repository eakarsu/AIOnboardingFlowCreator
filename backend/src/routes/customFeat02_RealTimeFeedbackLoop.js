...
// Video/audio captures during onboarding sessions; AI transcribes and surfaces common questions (e.g., "Where\'s the bathroom?" → add to FAQ) and sentiment (frustration indicators)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIOnboardingFlowCreator / cf-real-time-feedback-loop',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_real_time_feedback_loop`;
(async () => {
  try {
    await pool.query(`
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Real-time feedback loop'.
Context: Video/audio captures during onboarding sessions; AI transcribes and surfaces common questions (e.g., "Where\'s the bathroom?" → add to FAQ) and sentiment (frustration indicators)
...
If input is insufficient, populate sensible defaults and note assumptions in summary.`;
    ...
    console.error('cf-real-time-feedback-loop/run error:', err);
