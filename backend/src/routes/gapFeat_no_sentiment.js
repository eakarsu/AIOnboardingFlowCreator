...
// Gap Feature: No \'/sentiment
// No /sentiment-trend-ai (track sentiment across onboarding journey)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIOnboardingFlowCreator / gap-no-sentiment',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No \'/sentiment'.\nProject: AIOnboardingFlowCreator.\nContext: No /sentiment-trend-ai (track sentiment across onboarding journey)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-sentiment', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-sentiment', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
