...
// Gap Feature: Missing dedicated checklist routes (only AI generation, not CRUD)
// Missing dedicated checklist routes (only AI generation, not CRUD)
// Project: AIOnboardingFlowCreator
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIOnboardingFlowCreator / gap-missing-dedicated-checklist-routes-only-ai-generat',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Missing dedicated checklist routes (only AI generation, not CRUD)'.\nProject: AIOnboardingFlowCreator.\nContext: Missing dedicated checklist routes (only AI generation, not CRUD)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['missing-dedicated-checklist-routes-only-ai-generat', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'missing-dedicated-checklist-routes-only-ai-generat', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['missing-dedicated-checklist-routes-only-ai-generat']
    );
