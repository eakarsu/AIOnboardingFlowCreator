...
// Gap (Non-AI): No RBAC granularity beyond admin/manager
// No RBAC granularity beyond admin/manager
import React, { useState } from 'react';

export default function GapNoRbacGranularityBeyondAdminManagerPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-rbac-granularity-beyond-admin-manager/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>No RBAC granularity beyond admin/manager</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>No RBAC granularity beyond admin/manager</p>
