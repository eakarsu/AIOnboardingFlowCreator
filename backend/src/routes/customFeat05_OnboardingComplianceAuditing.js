...
// Background process checks onboarding completion rates by department; flags compliance gaps (e.g., missing security training)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIOnboardingFlowCreator / cf-onboarding-compliance-auditing',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_onboarding_compliance_auditing`;
(async () => {
  try {
    await pool.query(`
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Onboarding compliance auditing'.
Context: Background process checks onboarding completion rates by department; flags compliance gaps (e.g., missing security training)
...
If input is insufficient, populate sensible defaults and note assumptions in summary.`;
    ...
    console.error('cf-onboarding-compliance-auditing/run error:', err);
