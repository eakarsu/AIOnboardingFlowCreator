...
// Gap (Non-AI): Limited reporting export (PDF/CSV)
// Limited reporting export (PDF/CSV)
import React, { useState } from 'react';

export default function GapLimitedReportingExportPdfCsvPage() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-limited-reporting-export-pdf-csv/run', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: 24, color: '#e5e7eb' }}>
      <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Limited reporting export (PDF/CSV)</h1>
      <p style={{ color: '#9ca3af', marginBottom: 16 }}>Limited reporting export (PDF/CSV)</p>
