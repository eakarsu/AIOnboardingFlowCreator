...
// Gap Feature: No \'/cohort
// No /cohort-comparison-ai (compare onboarding outcomes across cohorts)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIOnboardingFlowCreator / gap-no-cohort',
    },
    body: JSON.stringify({
...
router.post('/run', auth, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No \'/cohort'.\nProject: AIOnboardingFlowCreator.\nContext: No /cohort-comparison-ai (compare onboarding outcomes across cohorts)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-cohort', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-cohort', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
